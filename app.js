...
addEventListener('DOMContentLoaded', () => {
    function main() {
        ...
        if (time > iftar) {
            current_date = date_format(new Date(new Date(current_date).getTime() + 1000 * 60 * 60 * 24));
            [sehri, iftar] = data[current_date];
            sehri = new Date(current_date + ' ' + sehri).getTime();
            iftar = new Date(current_date + ' ' + iftar).getTime();
...
addEventListener('keydown', (e) => {
    //keycode of f or keycode of ফ
    if (e.keyCode === 70 || e.keyCode === 231) {
        //fullscreen
        if (document.fullscreenElement) {
            document.exitFullscreen();
        } else {
